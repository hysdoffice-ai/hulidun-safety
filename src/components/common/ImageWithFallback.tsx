...
export function ImageWithFallback({ src, alt, className, imgClassName, fallbackLabel = "Image placeholder" }: ImageWithFallbackProps) {
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-md bg-gradient-to-br from-slate-700 via-slate-900 to-black", className)}>
      {!failed ? (
        <img src={src} alt={alt} onError={() => setFailed(true)} className={cn("h-full w-full object-cover", imgClassName)} />
      ) : (
